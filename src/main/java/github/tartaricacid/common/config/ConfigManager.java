...
package github.tartaricacid.common.config;
...
public class ConfigManager
{
    ...
    {
        FMLLog.getLogger().debug("Loading @Config anotation data");
        for (ASMDataTable.ASMData target : data.getAll(Config.class.getName()))
        {
            ...
            Config.Type type;
            try {
                Field value = ModAnnotation.EnumHolder.class.getDeclaredField("value");
                value.setAccessible(true);
                type = tholder == null ? Config.Type.INSTANCE : Config.Type.valueOf((String) value.get(tholder));
            } catch (Exception e) {
                FMLLog.getLogger().error("Cannot load EnumHolder data", e);
     ...
     *
     * Note, that this method is being called by the {@link cpw.mods.fml.common.FMLModContainer}, so the mod needn't call it in init().
     *
    ...
    {
        FMLLog.getLogger().debug("Attempting to inject @Config classes into {} for type {}", modid, type);
        ...
        for (ASMData targ : map.get(type))
        {
            ...
            {
                FMLLog.getLogger().error("An error occurred trying to load a config for {} into {}", targ.getClassName(), e);
